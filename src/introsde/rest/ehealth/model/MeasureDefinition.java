...
@NamedQueries(
{
	...
	@NamedQuery(name = "MeasureDefinition.findByName", query = "SELECT m FROM MeasureDefinition m WHERE m.measureName = :mName")
})
@XmlRootElement
public class MeasureDefinition implements Serializable {
	...
	public static List<String> getAllMeasureType() {
	      ...
	      List<MeasureDefinition> list = em.createNamedQuery("MeasureDefinition.findAll", MeasureDefinition.class).getResultList();
	        ...
	        {
	            listString.add(md.getMeasureName());
	        }
	        LifeCoachDao.instance.closeConnections(em);
	        return listString;
	    }
	...
	public static MeasureDefinition getMeasureDefinitionByName(String mType) {
		EntityManager em = LifeCoachDao.instance.createEntityManager();
		MeasureDefinition m = em.createNamedQuery("MeasureDefinition.findByName", MeasureDefinition.class)
				.setParameter("mName", mType)
				.getSingleResult();
		LifeCoachDao.instance.closeConnections(em);
		return m;
	}
