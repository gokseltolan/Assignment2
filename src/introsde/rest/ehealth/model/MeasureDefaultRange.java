...
public class MeasureDefaultRange implements Serializable {
	...
	@TableGenerator(name = "sqlite_range", table = "sqlite_sequence", pkColumnName = "name", valueColumnName = "seq", pkColumnValue = "MeasureDefaultRange")
	@Column(name = "idRange")
	private int idRange;
	...
	public static List<MeasureDefaultRange> getAll() {
		EntityManager em = LifeCoachDao.instance.createEntityManager();
		List<MeasureDefaultRange> list = em.createNamedQuery("MeasureDefaultRange.findAll", MeasureDefaultRange.class)
				.getResultList();
		LifeCoachDao.instance.closeConnections(em);
		return list;
	}
